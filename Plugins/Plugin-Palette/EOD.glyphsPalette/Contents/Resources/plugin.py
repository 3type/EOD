...
import os, time, objc, subprocess
...
class EOD(PalettePlugin):
		...
		for glyphUni in thisUniList:
			if glyphUni in self.idsDict.keys():
				...
				for partName, noteList in partNameDict.items():
					...
					rootName = partName.removeprefix('_part.u').split('-',1)[0]
					partNameStr = partNameStr + '/' + partName + ' '

					if not font.glyphs[partName]:
						if font.glyphs[rootName]:
							newGlyph = font.glyphs[rootName].copy()
						else:
							newGlyph = GSGlyph()
						newGlyph.name = partName
	...
	def siblingButton_(self, sender):
		...
		for xUni in searchRange:
			matchType, matchPart, matchNote = self.isSlibing(thisGlyphUni, xUni)
		...
		partStrA = getRandomStr(partListA, self.textFieldPartAAmount.intValue())
		partStrB = getRandomStr(partListB, self.textFieldPartBAmount.intValue())
		# print('A侧部件:', partStrA)
		# print('B侧部件:', partStrB)
	...
	def loadIDSButton_(self, sender):
		...
		'''
		# if self.idsDataPopupButton.indexOfSelectedItem() == 0:
		# 	self.readPdata()
		# if self.idsDataPopupButton.indexOfSelectedItem() == 1:
		# 	self.readPdata(option=1)
		# if self.idsDataPopupButton.indexOfSelectedItem() == 2:
		# 	self.readPdata(option=2)
	...
	def doneList(self, update=False, allList=False):
		...
		for glyph in Glyphs.font.glyphs:
			if allList:
				...
				totalStrokes = self.idsDict.get(glyph.unicode, {'totalStrokes': 0}).get('totalStrokes')
				if totalStrokes <= 3:
					targetStrokes = 1
				else:
